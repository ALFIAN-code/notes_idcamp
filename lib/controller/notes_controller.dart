...
class NotesController {
  List<Note> notes = [
    Note(
        ...
        title: 'First Note',
        content:
            """Pemrograman adalah proses menulis instruksi untuk komputer untuk melakukan tugas tertentu. Instruksi ini ditulis dalam bahasa yang disebut bahasa pemrograman. Bahasa pemrograman adalah seperangkat aturan yang digunakan untuk berkomunikasi dengan komputer.
...
Ada banyak jenis pemrograman yang berbeda, masing-masing dengan tujuannya sendiri. Beberapa jenis pemrograman yang paling umum adalah:""")
  ...
  List<Note> getHighlight() {
    List<Note> highlightNote = notes
        .where((element) => element.category == Category.Highlight)
        .toList();
    return highlightNote;
  }

  List<Note> getImportant() {
    List<Note> highlightNote = notes
        .where((element) => element.category == Category.Important)
        .toList();
    return highlightNote;
  }
}
